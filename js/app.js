...
(function () {
  ...
  async function openPdfForPreview(file) {
    ...
    try {
      ...
      activePdfDoc = doc;
      // Catatan: `buf` di atas ditransfer ke worker PDF.js saat loadDocument()
      // dan menjadi "detached" (tidak bisa dipakai/disimpan lagi) setelah itu.
      // Untuk penyimpanan ke IndexedDB nanti, salinan segar dibaca ulang
      // langsung dari `file` saat tombol "Impor Peta" ditekan (lihat di bawah).
      ...
      console.error(err);
      modalProgress.textContent = 'Gagal membuka berkas: ' + (err && err.message ? err.message : 'pastikan ini berkas .pdf yang valid') + '.';
    }
  }
  ...
  modalImport.addEventListener('click', async () => {
    ...
    try {
      ...
      // Baca ulang berkas PDF sebagai ArrayBuffer segar di sini — buffer yang
      // dipakai untuk loadDocument() sebelumnya sudah "detached" (ditransfer
      // ke worker PDF.js) dan tidak bisa lagi disimpan ke IndexedDB.
      const pdfArrayBuffer = await pendingFile.arrayBuffer();
      ...
      try {
        ...
        throw new Error('Gagal menyimpan ke penyimpanan lokal browser (' + (err && err.message ? err.message : err) + '). Coba bersihkan sebagian ruang penyimpanan perangkat, atau impor ke browser lain.');
      }
      ...
      console.error(err);
      modalProgress.textContent = 'Gagal mengimpor peta: ' + (err && err.message ? err.message : 'kesalahan tidak diketahui') + '. Coba lagi.';
    } finally {
