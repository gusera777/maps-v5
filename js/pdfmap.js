...
const GuseraPDF = (() => {
   ...
   * Jika render gagal atau browser tidak berhasil membuat blob pada resolusi
   * yang diminta (umum terjadi di beberapa browser mobile untuk PDF vektor
   * yang padat, mis. peta blok kebun dengan ratusan label & arsiran), fungsi
   * ini otomatis mencoba ulang sekali dengan resolusi lebih rendah sebelum
   * benar-benar gagal, agar impor tetap berhasil (dengan kualitas sedikit
   * lebih rendah) alih-alih menampilkan error tanpa keterangan.
   */
  async function renderPageToBlob(doc, pageNum, targetMaxDim = 2200) {
    try {
      return await renderAtResolution(doc, pageNum, targetMaxDim);
    } catch (err) {
      if (targetMaxDim <= 900) throw err; // sudah di resolusi rendah, jangan coba lagi
      console.warn('Render PDF gagal di resolusi ' + targetMaxDim + 'px, mencoba ulang lebih rendah:', err);
      return renderAtResolution(doc, pageNum, Math.round(targetMaxDim * 0.6));
    }
  }

  async function renderAtResolution(doc, pageNum, targetMaxDim) {
    ...
    try {
      ...
      throw new Error('Gagal me-render halaman PDF (' + (err && err.message ? err.message : err) + ')');
    }

    let blob = await new Promise((resolve) => canvas.toBlob(resolve, 'image/png'));
    if (!blob) {
      // Sejumlah browser (terutama Safari/WebView versi lama) kadang gagal
      // menghasilkan blob PNG langsung untuk canvas besar — fallback ke
      // toDataURL lalu ubah manual menjadi Blob.
      try {
    ...
    if (!blob) {
      throw new Error('Browser gagal membuat gambar dari halaman PDF pada resolusi ' + canvas.width + 'x' + canvas.height + 'px.');
    }
